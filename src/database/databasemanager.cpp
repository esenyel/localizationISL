...
cv::Mat DatabaseManager::createLocationMatrix( cv::Mat points){

    int location_number = points.rows;
    cv::Mat location_matrix(location_number, 2, CV_64F);

    std::cout << "location_number" << location_number << std::endl;

    for(int i=0; i<location_number; i++){

        int member_no = points.at<int>(i,0);
        QSqlQuery query(QString("select* from basepoint where id = %1").arg(member_no));
        query.next();

        location_matrix.at<double>(i,0)=query.value(7).toFloat();
        location_matrix.at<double>(i,1)=query.value(8).toFloat();
...
cv::Mat getBubbleInvariant(int member_id, int number_of_filters){

    cv::Mat bubble_invariant;
    QSqlQuery query(QString("select* from basepoint where id = %1").arg(member_id));

    QByteArray invariant_array = query.value(5).toByteArray();
    bubble_invariant = DatabaseManager::byteArray2Mat(invariant_array);

    return bubble_invariant;
