...
cv::Mat bubbleProcess::mstCalculateInvariants(std::vector <bubblePoint> bubble, DFCoefficients coeff, int harmonic1, int harmonic2)
{
    ...
    for (int m=0;m<M;m++){
        ...
        {
            if (m==0)
                result.at<float>(0,index) = coeff.a[m][n]*coeff.a[m][n]+coeff.c[m][n]*coeff.c[m][n];
            else
                result.at<float>(0,index) = coeff.a[m][n]*coeff.a[m][n]+coeff.b[m][n]*coeff.b[m][n]+coeff.c[m][n]*coeff.c[m][n]+coeff.d[m][n]*coeff.d[m][n];
            index++;
        }
    }
    return result.clone();
